...
void Game::ProcessInput(GLfloat dt,SDL_Event& e,InputManager& input)
{
...
void Game::Render()
{
	if (this->State == GAME_ACTIVE)
	{
		...
		Effects->Render(SDL_GetTicks()/1000);
...
{
	if (this->Level == 0)
		this->Levels[0].Load("src/Level/one.lvl", this->Width, this->Height * 0.5f);
	else if (this->Level == 1)
		this->Levels[1].Load("src/Level/two.lvl", this->Width, this->Height * 0.5f);
	else if (this->Level == 2)
		this->Levels[2].Load("src/Level/three.lvl", this->Width, this->Height * 0.5f);
	else if (this->Level == 3)
		this->Levels[3].Load("src/Level/four.lvl", this->Width, this->Height * 0.5f);
...
void Game::DoCollisions()
{
		...
		if (!box.Destroyed)
		{
			...
			if (std::get<0>(collision)) // If collision is true
			{
				...
				if (!box.IsSolid)
				{
					...
					SoundEngine->play2D("src/Audio/bleep.mp3", GL_FALSE);
				}
				else
				{
					ShakeTime = 0.05f;
					Effects->Shake = GL_TRUE;
					SoundEngine->play2D("src/Audio/solid.wav", GL_FALSE);
				}
	...
	for (PowerUp &powerUp : this->PowerUps)
	{
		if (!powerUp.Destroyed)
		{
			...
			{	// Collided with player, now activate powerup
				...
				SoundEngine->play2D("src/Audio/powerup.wav", GL_FALSE);
			}
		}
	}
	...
	if (!Ball->Stuck && std::get<0>(result))
	{
		...
		Ball->Velocity.y = -1 * abs(Ball->Velocity.y); // Fix sticky paddle

		// If Sticky powerup is activated, also stick ball to paddle once new velocity vectors were calculated
		Ball->Stuck = Ball->Sticky;

		SoundEngine->play2D("src/Audio/bleep.wav", GL_FALSE);
	}
}
